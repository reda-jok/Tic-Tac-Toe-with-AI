...
def minimax(
    move:Move, maximizer:Mark, choosing_highest_score:bool = False 
    )-> int:
    if move.after_state.game_over:
        return move.after_state.evaluate_score(maximizer)
    
    return(max if choosing_highest_score else min)(
        minimax(next_move, maximizer, not choosing_highest_score)
